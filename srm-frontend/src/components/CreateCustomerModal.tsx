...
interface CreateCustomerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CustomerSummary) => void;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div>
            <h2 className="text-lg font-bold text-slate-900">新建客户档案</h2>
            <p className="text-sm text-slate-500">录入新的潜在客户信息</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <label className="text-xs font-medium text-slate-700">客户名称 (英文)</label>
                <input
                    required
                    name="name"
                    value={formData.name}
                    onChange={handleChange}
                    placeholder="例如: TechGlobal Electronics Inc."
                ...
                <div className="space-y-2">
                <label className="text-xs font-medium text-slate-700">客户名称 (中文)</label>
                <input
                    required
                    name="localName"
                    value={formData.localName}
                    onChange={handleChange}
                    placeholder="例如: 泰科全球电子有限公司"
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all text-sm"
                />
                </div>
          ...
          <div className="space-y-4">
             <h3 className="text-sm font-bold text-slate-900 border-l-4 border-brand-500 pl-2">业务需求 (Requirements)</h3>
             <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-medium text-slate-700">主要需求类型</label>
                  <select
                    name="businessType"
                  ...
                  >
                     ...
                     {['海运出口 (Ocean Export)', '海运进口 (Ocean Import)', '空运 (Air)', '多式联运 (Multimodal)', '仓储配送 (Warehousing)', '报关 (Customs)'].map(t => (
                ...
                <div className="col-span-2 space-y-2">
                  <label className="text-xs font-medium text-slate-700">需求概述</label>
                  <input
                    name="advantage"
                    value={formData.advantage}
                    onChange={handleChange}
                    placeholder="例如: 每月50TEU美西货量"
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all text-sm"
                  />
                </div>
             </div>
